...
pub struct Moddifier {
    pub parameter: tank::tank::WaterParameter,
    pub moddifier: f64, //this number will usually be reduced as a cost of keeping the fish
}

impl Moddifier {
    pub fn new(parameter: tank::tank::WaterParameter, moddifier: f64) -> Self {
        Moddifier { 
            parameter, 
...
pub struct Fish {
    ...
    pub tier: FishTier,
    pub fish_traits: Vec<tank::traits::Trait>,
...
impl Fish {
    pub fn new(species: &game_data::registry::FishSpecies) -> Fish {
        ...
        Fish {
            ...
            fish_traits: species.traits.iter().map(|t| {
                tank::traits::Trait::new(
                    tank::traits::TraitNames::from_str(&t.trait_name),
                    t.multiplier,
                    t.weight,
                )
            }).collect(),
            ...
            moddifiers: species.modifiers.iter().map(|m| {
                Moddifier::new(
                    tank::tank::WaterParameter::from_str(&m.parameter),
                    m.modifier,
                )
            }).collect(),
            hunger: 50.0,
    ...
    */
    pub fn calculate_wellness(&mut self, water: &tank::tank::WaterParameters) {
...
pub fn parameter_score(&self, parameter_value: f64, range: &ParameterRange) -> f64 {
    ...
    pub fn status_check(&mut self) {
        self.status = match self.wellness {
            s if s >= game_data::constants::WELLNESS_THRIVING => FishStatus::Thriving,
            s if s >= game_data::constants::WELLNESS_HEALTHY  => FishStatus::Healthy,
            s if s >= game_data::constants::WELLNESS_NEUTRAL  => FishStatus::Neatural, // Fixed spelling from your enum
            s if s >= game_data::constants::WELLNESS_SICK     => FishStatus::Sick,
            _                                      => FishStatus::Dead,
        }
    }
    ...
    pub fn calculate_hunger(&mut self) {
        ...
        let drain = match self.tier {
            FishTier::Nano         => game_data::constants::HUNGER_DRAIN_NANO,
            FishTier::Community    => game_data::constants::HUNGER_DRAIN_COMMUNITY,
            FishTier::Tropical     => game_data::constants::HUNGER_DRAIN_TROPICAL,
            FishTier::Predator     => game_data::constants::HUNGER_DRAIN_PREDATOR,
            FishTier::RiverMonster => game_data::constants::HUNGER_DRAIN_RIVER_MONSTER,
        };
    ...
    pub fn eat(&mut self, food_level: f64) {
        ...
        let restore = game_data::constants::BASE_FOOD_RESTORE * food_level.powf(1.5);
