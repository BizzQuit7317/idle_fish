...
fn window_conf() -> Conf {
    Conf {
        ...
        window_width: game_data::constants::WINDOWS_DEFAULT_WIDTH,
        window_height: game_data::constants::WINDOWS_DEFAULT_LENGTH,
...
async fn main() {
    let mut game_state: Option<systems::game_state::GameState> = None; //initiate the game
    let mut tank_sprites = rendering::sprites::TankSprites::new(); //allow the fishes to roam freely and beautifully
    let mut current_page = util::ui_helper::GamePage::MainMenu;
    let mut last_page = util::ui_helper::GamePage::MainMenu;
    ...
    loop {
        match current_page {
            util::ui_helper::GamePage::MainMenu => {
                match screens::menu::draw_main_menu() {
                    screens::menu::MenuChoice::NewGame => {
                        game_state = Some(systems::game_state::GameState::new());
                    ...
                    screens::menu::MenuChoice::Continue => {
                        game_state = Some(util::file_control::load_game_json());
                        if let Some(gs) = &mut game_state {
            ...
            util::ui_helper::GamePage::Game => {
                last_page = util::ui_helper::GamePage::Game;
                            ...
                            if let Some(species) = gs.fish_registry.fish.get(index) {
                                if gs.tank.fish.len() < gs.tank.max_fish as usize {
                                    if gs.economy.can_afford(gs.player.current_prestige, species) {
                                        ...
                                        gs.tank.fish.push(tank::fish::Fish::new(species));
                ...
                tank_sprites.update();
                tank_sprites.draw();
                //util::debug::draw_debug_zones();
                //util::debug::draw_debug_grid();
            },
            util::ui_helper::GamePage::Settings => {
