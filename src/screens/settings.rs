...
pub fn draw_settings_menu(
    last_page: &ui::GamePage,
    state: &mut SettingsState,
    active_tab: &SettingTab,
    gameState: Option<&game_state::GameState>,
) -> settingChoice {
    let sw = screen_width();
    ...
    if ui::draw_button_box(sw * 0.015, tb_btn_y, tb_btn_w, tb_btn_h, BUTTON_NEUTRAL, "Back", BLACK) {
        match last_page {
            &ui::GamePage::MainMenu => return settingChoice::MainMenu,
            &ui::GamePage::Game     => return settingChoice::GameMenu,
            &ui::GamePage::Settings => {} // nowhere to go back to
        }
    }
    ...
    match active_tab {
        ...
        &SettingTab::PlayerStats => {
            if let Some(gs) = gameState {
                ...
                let row_text_h = row_h * 0.9; // ui::draw_text_box uses h*0.5 for font
