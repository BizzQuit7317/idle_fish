...
pub enum HudAction {
    ...
    StoreScrollDown,
    ChangeWater,
    TestChangeStat(tank::tank::WaterParameter, bool),
    DebugShiftStatRight,
    DebugShiftStatLeft,
...
pub fn draw_main_hud(current_game_state: &systems::game_state::GameState, active_tab: &BottomTab) -> HudAction {
    ...
    let side_x = 0.0;
    let side_y = sh * 0.125;
    let side_w = sw * game_data::constants::STAT_AREA_WIDTH;
    let side_h = sh * game_data::constants::STAT_AREA_HEIGHT;
    ...
    for (i, (text, color)) in stats.iter().enumerate() {
        ...
        util::ui_helper::draw_stat(row_inset_x, ry, row_inset_w, row_h, text, *color);
    }

    // Side panel outer border
    draw_rectangle_lines(side_x, side_y, side_w, side_h, 5.0, game_data::constants::AREA_BORDER_COLOUR);

    // =====================================================================
    // TANK — unchanged position/size, just retained
    // =====================================================================
    util::ui_helper::draw_tank(sw * 0.25, sh * 0.125, sw * game_data::constants::TANK_WIDTH, sh * game_data::constants::TANK_HEIGHT);
    ...
    let bottom_h = sh * game_data::constants::BOTTOM_AREA_HEIGHT;
    let tab_strip_h = sh * game_data::constants::BOTTOM_TAB_AREA_HEIGHT;
    ...
    draw_rectangle_lines(0.0, bottom_top, sw, bottom_h, 5.0, game_data::constants::AREA_BORDER_COLOUR);
    ...
    match active_tab {
            ...
            if fish_count == 0 {
                ...
                for (i, fish) in current_game_state.tank.fish.iter().enumerate() {
                    ...
                    for (r, text) in rows.iter().enumerate() {
                        ...
                        util::ui_helper::draw_stat(cx, ry, col_w, row_inner_h, text, BLACK);
                    }
        ...
        &BottomTab::Testing => {
            ...
            util::ui_helper::draw_stat(c1x, c1_inner_top, col_w, sh * 0.035,
            ...
            util::ui_helper::draw_stat(c2x, c2_inner_top, col_w, sh * 0.035,
            ...
            util::ui_helper::draw_stat(c3x, c3_inner_top, col_w, sh * 0.035,
                "Debug Helpers", BLACK);
